/*
MCUFRIEND TFT + NEO-6M + MEGA + UNO SCREW SHIELD = SOMEWHAT CHEAPEST/SIMPLE STRATUM 1  Single DST GLOBAL LO-Res PRECISION GPS CLOCK
 ...
 ** Version 2   ** 
 ...
 - Enabling United Kingdoms DST when month data is validated.(line 246).Add your own by this line example.
...
//const int32_t zone_hours= +8L; 	//Fixed Timezone for non-Daylight Saving zone,will always display this timezone only.Defaulted variable for GPS clock.

float zone_hours;//For Global decoding,when coordinate is lock decode its timezone according to the fairly localised fixed algorithm(lo-res) automatically,Example:UTC12:45=+12.75 zone_hours convert to second +- with SAT Clock which always transmit UTC 0 time and convert back to Local HH:MM:SS format. 
...
{ 
  LAT=fix.latitude();LNG=fix.longitude();
	...
	if(LAT>=  60    && LAT <=  63     && LNG <=  3.7   && LNG >= -7.5    && fix.dateTime.month>3 && fix.dateTime.month<11	 ||LAT>=  54       && LAT<=  60     && LNG <=  3.7    && LNG >= -11.25  && fix.dateTime.month>3 && fix.dateTime.month<11 ||LAT>=  50      && LAT <=  54     && LNG <=  2.3    && LNG >= -11.25 && fix.dateTime.month>3 && fix.dateTime.month<11 ){ zone_hours=+1L;}else{zone_hours=+0L;}//UTC±0.R4,5&7/20,**UK DST** 	
	if(LAT>=  54    && LAT <=  57.5   && LNG <=  7     && LNG >=  3.7    ||LAT>=  45     && LAT <=  50     && LNG <= -5      && LNG >= -7.5    ||LAT>=  0       && LAT <=  20     && LNG <= -5      && LNG >= -18.75  ){ zone_hours=+0L;}//UTC±0.R6,8,9/20	
  ...
  tft.setCursor(36, 218);tft.setTextColor(WHITE);tft.print("Timezone     : ");tft.setTextColor(GREEN);tft.print(zone_hours);tft.print(" UTC");
	}
	
  

void loop(){if ( gps.available( serialgps )) {fix = gps.read();fixCount++;
if (fixCount >= 5){fixCount = 0; if (fix.valid.time)adjustTime(fix.dateTime);printGPSdata();}}}
...
{ 
  ...
  tft.setCursor(18, 46);
  tft.print(F("LOC : "));
  if (fix.valid.location) {tft.setTextColor(GREEN);
    tft.print( fix.latitude(), 5 ); 
    tft.print( ", " );
    tft.print( fix.longitude(), 5 );
    tft.println();}decodeTimezone();
