/*
MCUFRIEND TFT + NEO-6M + MEGA + UNO SCREW SHIELD = SOMEWHAT CHEAPEST/SIMPLE STRATUM 1  NON-DST GLOBAL LO-Res PRECISION GPS CLOCK
...
{ 
  LAT=fix.latitude();LNG=fix.longitude();
	...
	if(LAT>=  60    && LAT <=  63     && LNG <=  3.7   && LNG >= -7.5    ||LAT>=  54     && LAT <=  60     && LNG <=  3.7    && LNG >= -11.25  ||LAT>=  54      && LAT <=  57.5   && LNG <=  3.5    && LNG >= -7.5    ){ zone_hours=+0L;}//UTC±0.R4-6/20	
	if(LAT>=  50    && LAT <=  54     && LNG <=  2.3   && LNG >= -11.25  ||LAT>=  45     && LAT <=  50     && LNG <= -5      && LNG >= -7.5    ||LAT>=  0       && LAT <=  20     && LNG <= -5      && LNG >= -18.75  ){ zone_hours=+0L;}//UTC±0.R7-9/20	
