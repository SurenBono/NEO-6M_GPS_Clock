...
/*
- Video Preview https://youtu.be/XPjMDpkoY6w
- FX GPS Clock + Weekday decoder + Piezo + dht11 (9 wire + 4 direct pin).
- The sketch was to big to fit uno for piezo & dht11 so tried Mega with its extra hardware Serial ports advantages.
- New Hourly Piezo Chiming GPS Clock with local Temprature & Humidity sensor on Mega hardware Serial1.
- A rare sketch Since Weekday (Sun-Sat) are not included in NMEAGPS . Originally written by Eric Sitler 30August2016
- Mod to your timezone line 44
- Add prefered FX for led display & adjust to your Ledmatrix ic FC16_HW (Line22).
- https://pjrp.github.io/MDParolaFontEditor
*/
 
#include <SPI.h>
#include <math.h>
#include <dht11.h>                // Local Temprature & Humidity Sensor 0.4.1 by George Hadjikyriacou
...
#define dhtpin      A5     // Signal + Vcc 5V + GND  ((3wire), stick Vcc & GND pin direct 
...
String Mdn,DAYX,TIMEX,TIMEY,DATEX,UTC=" UTC+",LOGO = (char(127))+UTC+zone_hours+" "+(char(129)),TempHum,SAT=String(char(127));       
char dayxx[55],timexx[15],datexx[25],temphum[25],logo[25];
const char *GPS[5]={dayxx,timexx,datexx,temphum,logo};    
...
void printDHT11() {
  ...
  int humidity = DHT11.humidity;
   ...
   TempHum=String(" T:");TempHum +=String(temperature);TempHum +=String("°C ");
   TempHum +=String(" H:");TempHum +=String(humidity);TempHum +=String("%  ");
   
    Serial.print(",");Serial.print(TempHum);Serial.println(","+UTC+zone_hours);
}
...
{ 
 ...
 LOGO.toCharArray(logo, 15); TempHum.toCharArray(temphum, 25);
 
 Serial.print(TIMEZ);Serial.print(" ,");Serial.print(DAYX);Serial.print(" ,");Serial.print(DATEX);printDHT11();
 DAYX+="    "+ DATEX + "    " + temphum +" "+LOGO; DAYX.toCharArray(dayxx,55);
